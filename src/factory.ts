    ...
    while (!qaPassed) {
      ...
      try {
        ...
        if (!qaPassed) {
          ...
          await complete(state, fixRecord, [qaBuildPath], ['fix:explicit-issues', 'fix:regression-test', `fix-attempt:${state.fixAttempts}`, fix.summary, `model:${fix.metrics.model}`]);
        }
      } catch (error) { return fail(state, record, error); }
    }
