...
const definitions: Record<string, ContractSeed> = {
  ...
  FULL_BUILD: {
    ...
    evidenceRequired: [{ id: 'build:tests', description: 'Core tests pass in the generated workspace.', verifier: 'deterministic', required: true }, { id: 'build:typecheck', description: 'Generated workspace typecheck passes.', verifier: 'deterministic', required: true }, { id: 'build:dist', description: 'Production web-lite build exists.', verifier: 'deterministic', required: true }],
...
export function evaluateStageEvidence(contractValue: StageContract, observed: { artifacts: string[]; evidence: string[]; inputs?: string[]; artifactVersions?: Record<string, number> }): StageEvidenceResult {
  ...
  const missing = [...missingInputs, ...requiredArtifacts.filter((item) => !observed.artifacts.some((actual) => contractPathMatches(item, actual))), ...requiredEvidence.filter((item) => !observed.evidence.includes(item)), ...missingVersions];
